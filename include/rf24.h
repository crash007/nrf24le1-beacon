...
void radioBegin(){
	...
	//enable the spi interface and wait for it to come up
	...
	//reg = 0;
	//rf_write_register(FEATURE, &reg, 1);
	//rf_write_register(DYNPD, &reg, 1);
	//reg = _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT);
	//rf_write_register(STATUS, &reg, 1);
...
void read(void* buf, uint8_t len)
{
	...
	rf_read_rx_payload(buf, len);
	status = _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT);
	status = rf_write_register(RF_STATUS, &status, 1);
...
bool write(const void* buf, uint8_t len, const bool multicast)
{
	...
	if(multicast)
	{
		rf_write_tx_payload_noack(buf, len, true);
	}else
	{
		rf_write_tx_payload(buf, len, true);
	}

	while( ! ( rf_get_status()  & ( _BV(TX_DS) | _BV(MAX_RT) )));

	status = _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT);
	status = rf_write_register(RF_STATUS, &status, 1);
	if( status & _BV(MAX_RT)){
		rf_flush_tx(); //Only going to be 1 packet int the FIFO at a time using this method, so just flush
		return 0;
	}
...
void startListening(void){
	...
	config = _BV(RX_DR) | _BV(TX_DS) | _BV(MAX_RT);
	rf_write_register(RF_STATUS, &config, 1);
	rf_set_ce();
...
void stopListening(void){
	...
	rf_write_register(RF_CONFIG, &config, 1);
	rf_read_register(RF_EN_RXADDR, &config, 1);
	config |= _BV(RF_ERX_P0);
	rf_write_register(RF_EN_RXADDR, &config ,1);
}

void whatHappened(bool *tx_ok,bool *tx_fail,bool *rx_ready){

  radio_temp = rf_get_status(); //rf_write_register(STATUS, (uint8_t*)&radio_temp, 1 );

  // Report to the user what happened
  *tx_ok = radio_temp & _BV(TX_DS);
  *tx_fail = radio_temp & _BV(MAX_RT);
  *rx_ready = radio_temp & _BV(RX_DR);
...
void enableDynamicPayloads(){
	...
	radio_temp = rf_read_register_1_byte(FEATURE) | _BV(EN_DPL);
	rf_write_register(FEATURE, &radio_temp, 1);
	radio_temp = rf_read_register_1_byte(DYNPD) | _BV(DPL_P5) | _BV(DPL_P4) | _BV(DPL_P3) | _BV(DPL_P2) | _BV(DPL_P1) | _BV(DPL_P0);
	rf_write_register(DYNPD, & radio_temp, 1);
}
