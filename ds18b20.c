...
dserror_t ds18b20_read(int *temp) {
     ...
     */
    rtc2_stop();
    rtc2_configure(RTC2_CONFIG_OPTION_COMPARE_MODE_0_RESET_AT_IRQ, 6150); //65535=2 sек, 6150 = 187.68 ms
    rtc2_run();
    pwr_clk_mgmt_enter_pwr_mode_register_ret(); // sov 0.18768s, tiden att vänta för 10 bit upplösning.
    gpio_pin_val_read(DSPIN);

    //Sätt tillbaka SLEEP
    rtc2_stop();
    rtc2_configure(RTC2_CONFIG_OPTION_COMPARE_MODE_0_RESET_AT_IRQ, RTC2_SLEEP);
    rtc2_run();
